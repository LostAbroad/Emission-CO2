(function () {
	...
	//functions use the selectData array index + selected //year to access data for table, slider, and dropdown menu
	...
	function setMap(world, countryCenter, topo, selectedYear) {
        ...
        //creates basemap layers and controls
		...
		var worldLayer = L.geoJson(world)
				...
				var popup = L.popup({
					closeButton: true,
					autoClose: true
				}).setLatLng(map.getBounds().getCenter()).setContent('<p class= mainPopup1><b>The Story:</b> This map contains CO2 emission, GDP, Population Growth, as well as Population data spanning most countries from 1960 to 2014.</p><p class= mainPopup2><b>Search Selection:</B> There are two ways to search for a location: 1) Use the map search function on the left hand side of this popup 2) Select the country of interest by clicking on it in the table to the right (Note: you may need to click more than once for full funcitonality)</p><p class= mainPopup3><b>Scrollbar:</b>The scrollbar above will allow you to pick from a specific year.</p><p class= mainPopup4><b>Aerial View:</b> There is an option to change from aerial view to a basemap view.</p><p class= mainPopup5><b>Table Info:</b> A table to the right of this popup allows for the selection of various information via a dropdown menu.</p>').openOn(map);
	...
	function selectedColorScale(world, selectedYear, expressed) {
		...
		if (expressed == "GDP") {
			...
			color.range(d3.schemeOranges[9])
		}
...
//    function createChart(csvData, colorScale) {
//
...
//	
//		//add legend
//		var svg_legend = svg.append("g").attr("class", "svg_legend").attr("transform", "translate(" + map_dim.legend.left + "," + map_dim.legend.top + ")");
//		var tick_size = 20;
//		//scales for first variable... used by both map types
//		var x_ticks = x_quantile.quantiles().slice(0);
//		x_ticks.unshift(d3.min($.map(map_data, function (g) {
//			return parseFloat(g[x_var]);
//		})));
//		x_ticks.push(d3.max($.map(map_data, function (g) {
//			return parseFloat(g[x_var]);
//		})));
//	
